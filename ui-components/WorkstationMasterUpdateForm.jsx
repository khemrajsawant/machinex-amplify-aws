...
import { getWorkstationMaster } from "./graphql/queries";
import { updateWorkstationMaster } from "./graphql/mutations";
const client = generateClient();
export default function WorkstationMasterUpdateForm(props) {
  const {
    id: idProp,
    workstationMaster: workstationMasterModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [Per_Hrs_Rate, setPer_Hrs_Rate] = React.useState(
    initialValues.Per_Hrs_Rate
  );
  ...
  const resetStateValues = () => {
    const cleanValues = workstationMasterRecord
      ? { ...initialValues, ...workstationMasterRecord }
      : initialValues;
    setID(cleanValues.ID);
    setWorkstation(cleanValues.Workstation);
    setPer_Hrs_Rate(cleanValues.Per_Hrs_Rate);
  ...
  const [workstationMasterRecord, setWorkstationMasterRecord] = React.useState(
    workstationMasterModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getWorkstationMaster.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWorkstationMaster
        : workstationMasterModelProp;
      setWorkstationMasterRecord(record);
    };
    queryData();
  }, [idProp, workstationMasterModelProp]);
  React.useEffect(resetStateValues, [workstationMasterRecord]);
  const validations = {
    ID: [],
    Workstation: [],
    Per_Hrs_Rate: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          Per_Hrs_Rate: Per_Hrs_Rate ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateWorkstationMaster.replaceAll("__typename", ""),
            variables: {
              input: {
                id: workstationMasterRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WorkstationMasterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.Per_Hrs_Rate ?? value;
          }
          if (errors.Per_Hrs_Rate?.hasError) {
            runValidationTasks("Per_Hrs_Rate", value);
          }
          setPer_Hrs_Rate(value);
        }}
        onBlur={() => runValidationTasks("Per_Hrs_Rate", Per_Hrs_Rate)}
        errorMessage={errors.Per_Hrs_Rate?.errorMessage}
        hasError={errors.Per_Hrs_Rate?.hasError}
        {...getOverrideProps(overrides, "Per_Hrs_Rate")}
      ></TextField>
      <TextField
        label="Time stamp"
        isRequired={false}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workstationMasterModelProp)}
