...
import { getMachineMaster } from "./graphql/queries";
import { updateMachineMaster } from "./graphql/mutations";
const client = generateClient();
export default function MachineMasterUpdateForm(props) {
  const {
    id: idProp,
    machineMaster: machineMasterModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = machineMasterRecord
      ? { ...initialValues, ...machineMasterRecord }
      : initialValues;
    setID(cleanValues.ID);
    setMachine_float(cleanValues.Machine_float);
    setMachine_Name(cleanValues.Machine_Name);
    setMachine_Specification(cleanValues.Machine_Specification);
    setWorkstation(cleanValues.Workstation);
    setMachine_Per_Hrs_Rate(cleanValues.Machine_Per_Hrs_Rate);
  ...
  const [machineMasterRecord, setMachineMasterRecord] = React.useState(
    machineMasterModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getMachineMaster.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMachineMaster
        : machineMasterModelProp;
      setMachineMasterRecord(record);
    };
    queryData();
  }, [idProp, machineMasterModelProp]);
  React.useEffect(resetStateValues, [machineMasterRecord]);
  const validations = {
    ID: [],
    Machine_float: [],
    Machine_Name: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          Machine_Specification: Machine_Specification ?? null,
          Workstation: Workstation ?? null,
          Machine_Per_Hrs_Rate: Machine_Per_Hrs_Rate ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateMachineMaster.replaceAll("__typename", ""),
            variables: {
              input: {
                id: machineMasterRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "MachineMasterUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || machineMasterModelProp)}
