...
import { getAccountMasterForm } from "./graphql/queries";
import { updateAccountMasterForm } from "./graphql/mutations";
const client = generateClient();
export default function AccountMasterFormUpdateForm(props) {
  const {
    id: idProp,
    accountMasterForm: accountMasterFormModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = accountMasterFormRecord
      ? { ...initialValues, ...accountMasterFormRecord }
      : initialValues;
  ...
  const [accountMasterFormRecord, setAccountMasterFormRecord] = React.useState(
    accountMasterFormModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getAccountMasterForm.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAccountMasterForm
        : accountMasterFormModelProp;
      setAccountMasterFormRecord(record);
    };
    queryData();
  }, [idProp, accountMasterFormModelProp]);
  React.useEffect(resetStateValues, [accountMasterFormRecord]);
  const validations = {
    ID: [],
    Account_Name: [],
    Address: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateAccountMasterForm.replaceAll("__typename", ""),
            variables: {
              input: {
                id: accountMasterFormRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AccountMasterFormUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || accountMasterFormModelProp)}
