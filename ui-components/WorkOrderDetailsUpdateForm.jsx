...
import { getWorkOrderDetails } from "./graphql/queries";
import { updateWorkOrderDetails } from "./graphql/mutations";
const client = generateClient();
export default function WorkOrderDetailsUpdateForm(props) {
  const {
    id: idProp,
    workOrderDetails: workOrderDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = workOrderDetailsRecord
      ? { ...initialValues, ...workOrderDetailsRecord }
      : initialValues;
    ...
    setProd_Qty(cleanValues.Prod_Qty);
    setOut_Qty(cleanValues.Out_Qty);
  ...
  const [workOrderDetailsRecord, setWorkOrderDetailsRecord] = React.useState(
    workOrderDetailsModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getWorkOrderDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getWorkOrderDetails
        : workOrderDetailsModelProp;
      setWorkOrderDetailsRecord(record);
    };
    queryData();
  }, [idProp, workOrderDetailsModelProp]);
  React.useEffect(resetStateValues, [workOrderDetailsRecord]);
  const validations = {
    ID: [],
    Order_No: [],
    Sr_No: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          Prod_Qty: Prod_Qty ?? null,
          Out_Qty: Out_Qty ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateWorkOrderDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: workOrderDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "WorkOrderDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || workOrderDetailsModelProp)}
