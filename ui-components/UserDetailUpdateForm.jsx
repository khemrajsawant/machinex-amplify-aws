...
export default function UserDetailUpdateForm(props) {
  const {
    id: idProp,
    userDetail: userDetailModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [Access_Category, setAccess_Category] = React.useState(
  ...
  const resetStateValues = () => {
    const cleanValues = userDetailRecord
      ? { ...initialValues, ...userDetailRecord }
      : initialValues;
    setID(cleanValues.ID);
    setEmp_ID(cleanValues.Emp_ID);
    setUser_Name(cleanValues.User_Name);
    setAccess_Category(cleanValues.Access_Category);
  ...
  const [userDetailRecord, setUserDetailRecord] =
    React.useState(userDetailModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getUserDetail.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getUserDetail
        : userDetailModelProp;
      setUserDetailRecord(record);
    };
    queryData();
  }, [idProp, userDetailModelProp]);
  React.useEffect(resetStateValues, [userDetailRecord]);
  const validations = {
    ID: [],
    Emp_ID: [],
    User_Name: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          Access_Category: Access_Category ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateUserDetail.replaceAll("__typename", ""),
            variables: {
              input: {
                id: userDetailRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "UserDetailUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.Access_Category ?? value;
          }
          if (errors.Access_Category?.hasError) {
            runValidationTasks("Access_Category", value);
          }
          setAccess_Category(value);
        }}
        onBlur={() => runValidationTasks("Access_Category", Access_Category)}
        errorMessage={errors.Access_Category?.errorMessage}
        hasError={errors.Access_Category?.hasError}
        {...getOverrideProps(overrides, "Access_Category")}
      ></TextField>
      <TextField
        label="Time stamp"
        isRequired={false}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || userDetailModelProp)}
