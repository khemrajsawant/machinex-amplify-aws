...
import { getJobWorkOrder } from "./graphql/queries";
import { updateJobWorkOrder } from "./graphql/mutations";
const client = generateClient();
export default function JobWorkOrderUpdateForm(props) {
  const {
    id: idProp,
    jobWorkOrder: jobWorkOrderModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = jobWorkOrderRecord
      ? { ...initialValues, ...jobWorkOrderRecord }
      : initialValues;
  ...
  const [jobWorkOrderRecord, setJobWorkOrderRecord] = React.useState(
    jobWorkOrderModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getJobWorkOrder.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getJobWorkOrder
        : jobWorkOrderModelProp;
      setJobWorkOrderRecord(record);
    };
    queryData();
  }, [idProp, jobWorkOrderModelProp]);
  React.useEffect(resetStateValues, [jobWorkOrderRecord]);
  const validations = {
    ID: [],
    JWO_No: [],
    Drawing_float: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateJobWorkOrder.replaceAll("__typename", ""),
            variables: {
              input: {
                id: jobWorkOrderRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "JobWorkOrderUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || jobWorkOrderModelProp)}
