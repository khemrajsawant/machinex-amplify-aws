...
import { getJobWorkRecceiptDetails } from "./graphql/queries";
import { updateJobWorkRecceiptDetails } from "./graphql/mutations";
const client = generateClient();
export default function JobWorkRecceiptDetailsUpdateForm(props) {
  const {
    id: idProp,
    jobWorkRecceiptDetails: jobWorkRecceiptDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = jobWorkRecceiptDetailsRecord
      ? { ...initialValues, ...jobWorkRecceiptDetailsRecord }
      : initialValues;
    setID(cleanValues.ID);
    setInword_No(cleanValues.Inword_No);
    setJWI_Sr_No(cleanValues.JWI_Sr_No);
  ...
  const [jobWorkRecceiptDetailsRecord, setJobWorkRecceiptDetailsRecord] =
    React.useState(jobWorkRecceiptDetailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getJobWorkRecceiptDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getJobWorkRecceiptDetails
        : jobWorkRecceiptDetailsModelProp;
      setJobWorkRecceiptDetailsRecord(record);
    };
    queryData();
  }, [idProp, jobWorkRecceiptDetailsModelProp]);
  React.useEffect(resetStateValues, [jobWorkRecceiptDetailsRecord]);
  const validations = {
    ID: [],
    Inword_No: [],
    JWI_Sr_No: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ID: ID ?? null,
          Inword_No: Inword_No ?? null,
          JWI_Sr_No: JWI_Sr_No ?? null,
          JWO_No: JWO_No ?? null,
          PO_Ref: PO_Ref ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateJobWorkRecceiptDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: jobWorkRecceiptDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "JobWorkRecceiptDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || jobWorkRecceiptDetailsModelProp)}
