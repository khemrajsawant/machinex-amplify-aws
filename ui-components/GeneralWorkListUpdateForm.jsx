...
import { getGeneralWorkList } from "./graphql/queries";
import { updateGeneralWorkList } from "./graphql/mutations";
const client = generateClient();
export default function GeneralWorkListUpdateForm(props) {
  const {
    id: idProp,
    generalWorkList: generalWorkListModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [General_Work, setGeneral_Work] = React.useState(
    initialValues.General_Work
  );
  ...
  const resetStateValues = () => {
    const cleanValues = generalWorkListRecord
      ? { ...initialValues, ...generalWorkListRecord }
      : initialValues;
    setID(cleanValues.ID);
    setSr_No(cleanValues.Sr_No);
    setGeneral_Work(cleanValues.General_Work);
  ...
  const [generalWorkListRecord, setGeneralWorkListRecord] = React.useState(
    generalWorkListModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getGeneralWorkList.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getGeneralWorkList
        : generalWorkListModelProp;
      setGeneralWorkListRecord(record);
    };
    queryData();
  }, [idProp, generalWorkListModelProp]);
  React.useEffect(resetStateValues, [generalWorkListRecord]);
  const validations = {
    ID: [],
    Sr_No: [],
    General_Work: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ID: ID ?? null,
          Sr_No: Sr_No ?? null,
          General_Work: General_Work ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateGeneralWorkList.replaceAll("__typename", ""),
            variables: {
              input: {
                id: generalWorkListRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "GeneralWorkListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.General_Work ?? value;
          }
          if (errors.General_Work?.hasError) {
            runValidationTasks("General_Work", value);
          }
          setGeneral_Work(value);
        }}
        onBlur={() => runValidationTasks("General_Work", General_Work)}
        errorMessage={errors.General_Work?.errorMessage}
        hasError={errors.General_Work?.hasError}
        {...getOverrideProps(overrides, "General_Work")}
      ></TextField>
      <TextField
        label="Time stamp"
        isRequired={false}
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || generalWorkListModelProp)}
