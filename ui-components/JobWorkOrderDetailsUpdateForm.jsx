...
import { getJobWorkOrderDetails } from "./graphql/queries";
import { updateJobWorkOrderDetails } from "./graphql/mutations";
const client = generateClient();
export default function JobWorkOrderDetailsUpdateForm(props) {
  const {
    id: idProp,
    jobWorkOrderDetails: jobWorkOrderDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = jobWorkOrderDetailsRecord
      ? { ...initialValues, ...jobWorkOrderDetailsRecord }
      : initialValues;
  ...
  const [jobWorkOrderDetailsRecord, setJobWorkOrderDetailsRecord] =
    React.useState(jobWorkOrderDetailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getJobWorkOrderDetails.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getJobWorkOrderDetails
        : jobWorkOrderDetailsModelProp;
      setJobWorkOrderDetailsRecord(record);
    };
    queryData();
  }, [idProp, jobWorkOrderDetailsModelProp]);
  React.useEffect(resetStateValues, [jobWorkOrderDetailsRecord]);
  const validations = {
    ID: [],
    JWO_No: [],
    JWO_Sr_No: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateJobWorkOrderDetails.replaceAll("__typename", ""),
            variables: {
              input: {
                id: jobWorkOrderDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "JobWorkOrderDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || jobWorkOrderDetailsModelProp)}
