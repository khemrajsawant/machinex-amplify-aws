...
import { getJobWorkOrderOperationDetails } from "./graphql/queries";
import { updateJobWorkOrderOperationDetails } from "./graphql/mutations";
const client = generateClient();
export default function JobWorkOrderOperationDetailsUpdateForm(props) {
  const {
    id: idProp,
    jobWorkOrderOperationDetails: jobWorkOrderOperationDetailsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = jobWorkOrderOperationDetailsRecord
      ? { ...initialValues, ...jobWorkOrderOperationDetailsRecord }
      : initialValues;
    setID(cleanValues.ID);
    setJWO_No(cleanValues.JWO_No);
    setJWO_Sr_No(cleanValues.JWO_Sr_No);
    setDrawing_float(cleanValues.Drawing_float);
    setQty(cleanValues.Qty);
    setDel_Date(cleanValues.Del_Date);
    setWork_Details(cleanValues.Work_Details);
    setRate(cleanValues.Rate);
    setTotal_Cost(cleanValues.Total_Cost);
    setJWO_In_Qty(cleanValues.JWO_In_Qty);
    setBal_Qty(cleanValues.Bal_Qty);
    setRej_Qty(cleanValues.Rej_Qty);
  ...
  const [
    jobWorkOrderOperationDetailsRecord,
    setJobWorkOrderOperationDetailsRecord,
  ] = React.useState(jobWorkOrderOperationDetailsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getJobWorkOrderOperationDetails.replaceAll(
                "__typename",
                ""
              ),
              variables: { id: idProp },
            })
          )?.data?.getJobWorkOrderOperationDetails
        : jobWorkOrderOperationDetailsModelProp;
      setJobWorkOrderOperationDetailsRecord(record);
    };
    queryData();
  }, [idProp, jobWorkOrderOperationDetailsModelProp]);
  React.useEffect(resetStateValues, [jobWorkOrderOperationDetailsRecord]);
  const validations = {
    ID: [],
    JWO_No: [],
    JWO_Sr_No: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ID: ID ?? null,
          JWO_No: JWO_No ?? null,
          JWO_Sr_No: JWO_Sr_No ?? null,
          Drawing_float: Drawing_float ?? null,
          Qty: Qty ?? null,
          Del_Date: Del_Date ?? null,
          Work_Details: Work_Details ?? null,
          Rate: Rate ?? null,
          Total_Cost: Total_Cost ?? null,
          JWO_In_Qty: JWO_In_Qty ?? null,
          Bal_Qty: Bal_Qty ?? null,
          Rej_Qty: Rej_Qty ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateJobWorkOrderOperationDetails.replaceAll(
              "__typename",
              ""
            ),
            variables: {
              input: {
                id: jobWorkOrderOperationDetailsRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "JobWorkOrderOperationDetailsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || jobWorkOrderOperationDetailsModelProp)}
