...
import { getRejectionReport } from "./graphql/queries";
import { updateRejectionReport } from "./graphql/mutations";
const client = generateClient();
export default function RejectionReportUpdateForm(props) {
  const {
    id: idProp,
    rejectionReport: rejectionReportModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = rejectionReportRecord
      ? { ...initialValues, ...rejectionReportRecord }
      : initialValues;
  ...
  const [rejectionReportRecord, setRejectionReportRecord] = React.useState(
    rejectionReportModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getRejectionReport.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getRejectionReport
        : rejectionReportModelProp;
      setRejectionReportRecord(record);
    };
    queryData();
  }, [idProp, rejectionReportModelProp]);
  React.useEffect(resetStateValues, [rejectionReportRecord]);
  const validations = {
    ID: [],
    Report_No: [],
    Date: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateRejectionReport.replaceAll("__typename", ""),
            variables: {
              input: {
                id: rejectionReportRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "RejectionReportUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || rejectionReportModelProp)}
