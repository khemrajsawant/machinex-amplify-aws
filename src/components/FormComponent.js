...
import { createMaster, updateMaster, updateSelectedDropdown, } from "../redux/tableStateGenForm/master/masterReducer";
...
export default function FormComponent(props) {
    ...
    const primary_key_name = props.COMPONENTS.filter((c) => c.isPrimaryKey === true)[0].name;
    ...
    const assignMachinesToOperations = (operationsToAssign, machineCodeInput) => {
        ...
        const filteredData = OperationsData.filter((item) => item.Workstation === OperationMapper[machineCodeInput]);
        console.log("filteredData", filteredData, "operationsToAssign", operationsToAssign);
        let filteredOperations = operationsToAssign.filter((op) => filteredData.some((data) => data.Opn_Name === op));
    ...
    if (formName == "EARNING_DETAILS") {
        ...
        console.log("selectedValueWork_Order_Number", selectedValueWork_Order_Number);
        console.log("selectedValueMachine_Number", selectedValueMachine_Number);
        console.log(inputObject);
        var itemTemp = [];
        React.useEffect(() => {
            var itemTemp = selectedValueMachine_Number === undefined ||
        ...
        React.useEffect(() => {
            var itemTemp = selectedValueDrawing_Number === undefined ||
        ...
        React.useEffect(() => {
            var test_var = selectedValueWork_Order_Number === undefined ||
            ...
            var itemTemp = selectedValueWork_Order_Number === undefined ||
                selectedValueWork_Order_Number === null
                ? inputObject[0]
            ...
            console.log("itemTemp", itemTemp);
            var filtered_ops = assignMachinesToOperations(itemTemp, selectedValueMachine_Number);
        ...
        React.useEffect(() => {
            ...
            if (selectedValueOperation === undefined ||
                ...
                try {
                    google.script.run
                        ...
                        .getLiveInfo("earningDetailsPendingQty", selectedValueWork_Order_Number + "_" + selectedValueOperation);
                }
                catch (error) {
    ...
    if (formName == "JOB_WORK_ORDER_DETAILS") {
        ...
        var itemTemp = [];
        React.useEffect(() => {
            var itemTemp = selectedValue === undefined || selectedValue === null
                ? inputObject[0]
        ...
        var itemTemp = [];
        React.useEffect(() => {
            var itemTemp = selectedValuePO_Ref === undefined || selectedValuePO_Ref === null
                ? inputObject[0]
    ...
    if (formName == "JOB_WORK_RECEIPT_DETAILS") {
        ...
        var itemTemp = [];
        React.useEffect(() => {
            var itemTemp = selectedValue_JWO_No === undefined || selectedValue_JWO_No === null
                ? inputObject[0]
        ...
        React.useEffect(() => {
            var itemTemp = selectedValue_PO_Ref === undefined || selectedValue_PO_Ref === null
                ? inputObject[0]
    ...
    if (formName == "WORK_ORDER_DETAILS") {
        ...
        const setOriginalSeq = (k) => {
            ...
            const indexMap = new Map(originalSequence.map((item, index) => [item, index]));
    ...
    if (formName == "BILL_OF_PROCESS") {
        ...
        var itemTemp = [];
        React.useEffect(() => {
            var itemTemp = selectedValue === undefined ||
                selectedValue === null ||
                selectedValue === ""
                ? inputObject[0]
                : inputObject[selectedValue];
    ...
    if (formName == "DAILY_WORK_REPORT") {
        ...
        React.useEffect(() => {
            ...
            setValue("Report_No", "DWR-" + props.prefilled.Worker_No + `-${day}-${month}-${year}`);
            //   var itemTemp =
            //   selectedValue === undefined || selectedValue === null
            //     ? inputObject[0]
            //     : inputObject[selectedValue];
    ...
    const onSubmit = async (data) => {
        ...
        const existingValues = tableFormData[formName].map((item) => item[col_to_watch]);
            ...
            if (isMaster === false) {
                ...
                try {
                    ...
                    google.script.run
                        ...
                        .withFailureHandler((er) => { })
                        .getLiveInfoKey("1HzTapPUdISUKfjzudW9PAiIJAQ3iYpUylWngSyIyeNw", formName, "A:G", "keyExists", column_to_search);
                }
                catch (error) {
                    console.error(error); // You might send an exception to your error tracker like AppSignal
                    const processedData = processData(formName, input);
    ...
    return (_jsx(React.Fragment, { children: _jsxs("form", { onSubmit: handleSubmit(onSubmit), autoComplete: "off", children: [_jsx(Stack, { direction: props.direction, sx: [
                        { width: "100%" },
                        { border: 1.5 },
                        { borderRadius: "15px" },
                        { paddingLeft: "85%" },
                    ...
                    ], useFlexGap: true, flexWrap: "wrap", children: _jsx(Tooltip, { title: "Refresh this form", children: _jsxs(Button
                        // variant="contained"
                        , { 
                            // variant="contained"
                            onClick: () => {
                            ...
                            ], children: ["           ", _jsx(Icon, { icon: "ooui:reload" })] }) }) }), _jsxs(Stack, { direction: props.direction, sx: [
                        { width: "100%" },
                        { border: 1.5 },
                        { borderRadius: "15px" },
                        { padding: "1rem" },
                        ...
                        { "&:hover": { backgroundColor: "white", borderRadius: "15px" } },
                            ...
                            if (cmp.show == true) {
                                if (cmp.select === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(2) }, children: _jsx(TestAutocompleteForm, { control: control, label: cmp.label, nameprop: cmp.name, items: itemTemp[cmp.name] ? itemTemp[cmp.name] : itemTemp, helpertext: cmp.helpertext, formName: formName, defaultvalue: defaultValuesArray[cmp.name], disabled: cmp.isDisabled, required: cmp.required }) }) }, cmp.order));
                                }
                                else if (cmp.multiselect === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(2) }, children: _jsx(MultiSelectAutocomplete, { control: control, label: cmp.label, nameprop: cmp.name, items: itemTemp, helpertext: cmp.helpertext, formName: formName, defaultvalue: defaultValuesArray[cmp.label], disabled: cmp.isDisabled, required: cmp.required }) }) }, cmp.order));
                                }
                                else if (cmp.input === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(3) }, children: _jsx(ControllerCompTextField, { control: control, label: cmp.label, nameprop: cmp.name, helpertext: cmp.helpertext, defaultvalue: defaultValuesArray[cmp.label], disabled: cmp.isDisabled, form: cmp.form, required: cmp.required, loading: loading, customStyles: {
                                                    width: COMPONENTS_COLUMNSDATA.length > 0
                                    ...
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(3) }, children: _jsx(CompFormcontrolRadio, { control: control, label: cmp.label, nameprop: cmp.name, helpertext: cmp.helpertext, disabled: cmp.isDisabled, defaultvalue: defaultValuesArray[cmp.label], required: cmp.required }) }) }, cmp.order));
                                }
                                else if (cmp.checkbox === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(3) }, children: _jsx(ControllerCompCheckbox, { control: control, label: cmp.label, nameprop: cmp.name, helpertext: cmp.helpertext, disabled: cmp.isDisabled, defaultvalue: defaultValuesArray[cmp.label], required: cmp.required }) }) }, cmp.order));
                                }
                                else if (cmp.date === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(3) }, children: _jsx(ControllerCompDateField, { control: control, label: cmp.label, nameprop: cmp.name, helpertext: cmp.helpertext, disabled: cmp.isDisabled, required: cmp.required }) }) }, cmp.order));
                                }
                                else if (cmp.time === true) {
                                    return (_jsx(Box, { order: cmp.order, sx: { marginLeft: "1rem" }, children: _jsx(Stack, { spacing: 1, sx: { marginBottom: (theme) => theme.spacing(3) }, children: _jsx(ControllerCompTimeField, { control: control, label: cmp.label, nameprop: cmp.name, helpertext: cmp.helpertext, disabled: cmp.isDisabled, required: cmp.required, defaultvalue: defaultValuesArray[cmp.label] }) }) }, cmp.order));
                                }
                            }
                        }), _jsx(Box, { order: 20, display: "flex", justifyContent: "center", alignItems: "center", sx: { marginLeft: "1rem" }, children: enableButton === true && (_jsx(_Fragment, { children: _jsx(Button, { type: "submit", variant: "contained", disabled: loading, size: "small", sx: [
                                        { minWidth: (theme) => theme.spacing(15) },
                                        { maxHeight: (theme) => theme.spacing(5) },
                                    ], children: "Save" }) })) })] })] }) }, reloadFlag));
}
