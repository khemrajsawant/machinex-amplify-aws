...
import { GridRowModes, DataGrid, GridToolbar, GridToolbarQuickFilter, 
// GridToolbarContainer,
GridActionsCellItem, GridRowEditStopReasons, } from "@mui/x-data-grid";
// import {
//   randomCreatedDate,
//   randomTraderName,
//   randomId,
...
import { deleteMaster, updateMaster, getSelectedItem, } from "../redux/tableStateGenForm/master/masterReducer";
function EditToolbar(props) {
    ...
    //   }));
    // };
    return (_jsxs(Stack, { direction: "row", children: [_jsx(GridToolbar, {}), _jsx(GridToolbarQuickFilter, { sx: { marginLeft: "15rem" } })] }));
}
export default function DataGridTest(props) {
    ...
    const role = useSelector((state) => state.master.AUTH_DATA.ROUTES
    ...
    columns = [
        ...columns,
        {
            ...
            getActions: ({ id }) => {
                ...
                let selected_row = props.rows.filter((c) => c.ID == indexToIdMapping[id]);
                ...
                if (isInEditMode) {
                    return [
                        _jsx(GridActionsCellItem, { icon: _jsx(SaveIcon, {}), label: "Save", sx: {
                                color: "primary.main",
                            }, onClick: handleSaveClick(id) }),
                        _jsx(GridActionsCellItem, { icon: _jsx(CancelIcon, {}), label: "Cancel", className: "textPrimary", onClick: handleCancelClick(id), color: "inherit" }),
                    ];
                }
                return [
                    _jsx(GridActionsCellItem, { icon: _jsx(EditIcon, {}), label: "Edit", className: "textPrimary", onClick: handleEditClick(id), color: "inherit", disabled: isDisabled }),
                    _jsx(GridActionsCellItem, { icon: _jsx(DeleteIcon, {}), label: "Delete", onClick: handleDeleteClick(id), color: "inherit", disabled: isDisabled }),
                    _jsx(_Fragment, { children: isMaster === true && props.rows.length > 0 && (_jsx(_Fragment, { children: _jsx(Link
                            // to={props.rows[id-1].isServer?generatePath(id):alert("save before")}
                            , { 
                                // to={props.rows[id-1].isServer?generatePath(id):alert("save before")}
                                to: generatePath(id), relative: "path", style: linkStyles, children: _jsx(LinkIcon, {}) }) })) }),
                ];
            },
        },
    ];
    ...
    return (_jsx(Box, { sx: {
        ...
        }, children: _jsx(DataGrid, { rows: props.rows, columns: columns, density: "compact", editMode: "row", rowModesModel: rowModesModel, onRowModesModelChange: handleRowModesModelChange, onRowEditStop: handleRowEditStop, processRowUpdate: processRowUpdate, onRowClick: onRowClickHandler, loading: props.isLoading === true
                ? true : false, slots: {
                toolbar: EditToolbar,
            }, slotProps: {
                toolbar1: {
