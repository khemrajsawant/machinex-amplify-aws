...
const WorkOrderFormSingleItem = (props) => {
            ...
            if (enableSave === true) {
                selectedItem.Order_No &&
                    google.script.run
                        ...
                        .importDataSelectedItem("WORK_ORDER_DETAILS", selectedItem.Order_No);
            }
        }
        catch (error) {
            console.error(error); // You might send an exception to your error tracker like AppSignal
    ...
    const preparePostData = () => {
        const prepdata = {
            ...
            // ),
            WORK_ORDER_DETAILS: tableWorkOrderDetails.filter((c) => !(c.isServer && !c.isDeleted && !c.isNew && !c.isModified)),
        };
        let temp1 = {
            // WORK_ORDER: prepdata.WORK_ORDER.filter(
            //   (c:any) => !(!c.isServer && c.isDeleted && c.isNew && !c.isModified)
            // ),
            WORK_ORDER_DETAILS: prepdata.WORK_ORDER_DETAILS.filter((c) => !(!c.isServer && c.isDeleted && c.isNew && !c.isModified)),
        };
        let temp2 = {
            // WORK_ORDER: temp1.WORK_ORDER.filter(
            //   (c:any) => !(!c.isServer && c.isDeleted && !c.isNew && c.isModified)
            // ),
            WORK_ORDER_DETAILS: temp1.WORK_ORDER_DETAILS.filter((c) => !(!c.isServer && c.isDeleted && !c.isNew && c.isModified)),
        };
        ////console.log(temp2);
        return temp2;
    };
    ...
    //       // handlerSave();
    //     }
    return (_jsxs(Container, { maxWidth: "xl", sx: [{ marginTop: (theme) => theme.spacing(10) }], children: [open === true && _jsx(CustomizedBackdrop, { open: open }), notification.open === true && (_jsx(CustomizedSnackbars, { open: notification.open, severity: notification.severity, message: notification.message, onChange: setNotificationFalse, duration: 5000 })), _jsxs(Stack, { children: [_jsxs(Stack, { children: [_jsx(FormHeader, { headerName: "WORK_ORDER" }), _jsx(Stack, { children: _jsx(TextFieldFreeze, { COMPONENTS: WORK_ORDER.metaData, items: items.WORK_ORDER, direction: "row", headerValue: "MACHINE_MASTER", formName: WORK_ORDER.metaData[0].sheetname, prefilled: selectedItem, enableButton: false }) })] }), _jsxs(Stack, { children: [_jsx(FormHeader, { headerName: "WORK_ORDER_DETAILS" }), _jsxs(Stack, { children: [_jsx(FormComponent, { COMPONENTS: WORK_ORDER_DETAILS.metaData, items: items.BILL_OF_PROCESS, direction: "row", headerValue: "MACHINE_MASTER", formName: WORK_ORDER_DETAILS.metaData[0].sheetname, prefilled: { tableWorkOrderDetails }, enableButton: true, isMaster: props.isMaster, primarykey: selectedItem, onChange: onChangeHandler }), _jsx(DataGridCustomComponent, { isMaster: props.isMaster, data: tableWorkOrderDetails, columns: WORK_ORDER_DETAILS.columnsData, formName: WORK_ORDER_DETAILS.metaData[0].sheetname, onChange: () => setEnableSave(false) })] })] }), _jsx(Stack, { children: _jsx(Button, { variant: "outlined", disabled: enableSave, sx: [
                                { marginLeft: "10rem" },
                                { marginRight: "10rem" },
                                { marginTop: "2rem" },
