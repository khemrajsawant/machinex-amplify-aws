...
const SalesChallanSingleItem = (props) => {
    ...
    const setData = (k) => {
        ...
        dispatch(fetchMaster(k, "SALE_CHALLAN_DETAILS"));
    };
    useEffect(() => {
        try {
            if (enableSave === true) {
    ...
    return (_jsxs(Container, { maxWidth: "xl", sx: [{ marginTop: (theme) => theme.spacing(10) }], children: [open === true && _jsx(CustomizedBackdrop, { open: open }), notification.open === true && (_jsx(CustomizedSnackbars, { open: notification.open, severity: notification.severity, message: notification.message, onChange: setNotificationFalse, duration: 3000 })), _jsxs(Stack, { children: [_jsxs(Stack, { children: [_jsx(FormHeader, { headerName: "SALE_CHALLAN" }), _jsx(Stack, { children: _jsx(TextFieldFreeze, { COMPONENTS: SALE_CHALLAN.metaData, items: items.SALE_CHALLAN, direction: "row", headerValue: "MACHINE_MASTER", formName: SALE_CHALLAN.metaData[0].sheetname, enableButton: false, 
                                    // primarykey={{ Challan_No: nextSaleChallanID }}
                                    prefilled: selectedItem }) })] }), _jsxs(Stack, { children: [_jsx(FormHeader, { headerName: "SALE_CHALLAN_DETAILS" }), _jsxs(Stack, { children: [_jsx(FormComponent, { COMPONENTS: SALE_CHALLAN_DETAILS.metaData, items: items.SALE_CHALLAN_DETAILS, direction: "row", headerValue: "MACHINE_MASTER", formName: SALE_CHALLAN_DETAILS.metaData[0].sheetname, enableButton: true, primarykey: selectedItem, onChange: () => null }), _jsx(DataGridCustomComponent, { isMaster: props.isMaster, data: tableSaleChallanDetails, columns: SALE_CHALLAN_DETAILS.columnsData, formName: SALE_CHALLAN_DETAILS.metaData[0].sheetname, onChange: () => setEnableSave(false) })] })] }), _jsx(Stack, { children: _jsx(Button, { variant: "outlined", disabled: enableSave, sx: [
                                { marginLeft: "10rem" },
                                { marginRight: "10rem" },
                                { marginTop: "2rem" },
