...
const JobWorkReceipt = (props) => {
    ...
    const nextJobWorkReceiptID = useSelector((state) => state.master.NEXT_AVAILABLE_ID["JOB_WORK_RECEIPT"]);
    // handlers
    ...
    const onChangeHandler = (e) => {
        ...
        // });
        dispatch(localUpdateNextAvailableEmpID("JWI" + "0".repeat(6 - maxId.toString().length) + maxId, "JOB_WORK_RECEIPT"));
        setEnableSave(false);
    };
    ...
    return (_jsxs(Container, { maxWidth: "xl", sx: [{ marginTop: (theme) => theme.spacing(10) }], children: [open === true && _jsx(CustomizedBackdrop, { open: open }), notification.open === true && (_jsx(CustomizedSnackbars, { open: notification.open, severity: notification.severity, message: notification.message, onChange: setNotificationFalse, duration: 3000 })), _jsxs(Stack, { children: [_jsxs(Stack, { children: [_jsx(FormHeader, { headerName: "JOB_WORK_RECEIPT" }), _jsxs(Stack, { children: [_jsx(FormComponent, { COMPONENTS: JOB_WORK_RECEIPT.metaData, items: items.JOB_WORK_RECEIPT, direction: "row", headerValue: "MACHINE_MASTER", formName: JOB_WORK_RECEIPT.metaData[0].sheetname, enableButton: true, primarykey: { Inword_No: nextJobWorkReceiptID }, prefilled: { Inword_No: nextJobWorkReceiptID }, onChange: onChangeHandler }), _jsx(DataGridCustomComponent, { isMaster: props.isMaster, data: tableJobWorkReceipt, columns: JOB_WORK_RECEIPT.columnsData, pathname: location.pathname, formName: JOB_WORK_RECEIPT.metaData[0].sheetname, onChange: () => setEnableSave(false) })] })] }), _jsx(Stack, { children: _jsx(Button, { variant: "outlined", disabled: enableSave, sx: [
                                { marginLeft: "10rem" },
                                { marginRight: "10rem" },
                                { marginTop: "2rem" },
