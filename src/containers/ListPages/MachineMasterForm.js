...
const MachineMasterForm = (props) => {
    ...
    const { MACHINE_MASTER: MACHINE_MASTER } = useSelector((state) => state.master.APP_DATA);
    // states
    ...
    const tableMachineMaster = useSelector((state) => state.master.MACHINE_MASTER);
    ...
    const preparePostData = () => {
        const prepdata = {
            MACHINE_MASTER: tableMachineMaster.filter((c) => !(c.isServer && !c.isDeleted && !c.isNew && !c.isModified)),
        };
        let temp1 = {
            MACHINE_MASTER: prepdata.MACHINE_MASTER.filter((c) => !(!c.isServer && c.isDeleted && c.isNew && !c.isModified)),
        };
        let temp2 = {
            MACHINE_MASTER: temp1.MACHINE_MASTER.filter((c) => !(!c.isServer && c.isDeleted && !c.isNew && c.isModified)),
        };
        ////console.log(temp2);
        return temp2;
    };
    ...
    return (_jsxs(Container, { maxWidth: "xl", sx: [{ marginTop: (theme) => theme.spacing(10) }, { height: "100vh" }], children: [open === true && _jsx(CustomizedBackdrop, { open: open }), notification.open === true && (_jsx(CustomizedSnackbars, { open: notification.open, severity: notification.severity, message: notification.message, onChange: setNotificationFalse, duration: 3000 })), _jsxs(Stack, { children: [_jsxs(Stack, { children: [_jsx(FormHeader, { headerName: SECTION_NAMES.SECTION_1 }), _jsxs(Stack, { children: [_jsx(FormComponent, { COMPONENTS: MACHINE_MASTER.metaData, items: items.MACHINE_MASTER, direction: "row", headerValue: "MACHINE_MASTER", formName: MACHINE_MASTER.metaData[0].sheetname, enableButton: true, primarykey: selectedItem, isMaster: props.isMaster, onChange: onChangeHandler }), _jsx(DataGridCustomComponent, { isMaster: false, data: tableMachineMaster, columns: MACHINE_MASTER.columnsData, formName: MACHINE_MASTER.metaData[0].sheetname, onChange: () => setEnableSave(false) })] })] }), _jsx(Stack, { children: _jsx(Button, { variant: "outlined", disabled: enableSave, sx: [
                                { marginLeft: "10rem" },
                                { marginRight: "10rem" },
                                { marginTop: "2rem" },
